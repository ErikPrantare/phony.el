...
This package also depends on [[https://github.com/ErikPrantare/simulacrum.el][simulacrum.el]].  After having installed
that, add the following to your Emacs configuration:
...
Note also that ~:interactive~ defaults to ~nil~ when ~:contributes-to~
is provided.
